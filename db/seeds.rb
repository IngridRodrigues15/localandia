...
CharacterType.find_or_create_by(name: 'Espadachim') do |type|
  ...
  type.weapons = '<b>Espada afiada</b>: Causa 1d12 de dano<br><b>Escudo</b>: Absorve completamente o dano de armas pequenas e até 50% de armas grandes, com teste de agilidade. Pode absorver completamente dano de armas grandes com teste de força'
  type.mana = 2
end
...
Item.find_or_create_by(name: 'Espada afiada') do |item|
  item.kind = 'Espadachim'
  item.price = 200
  item.description = 'causa 1d12 de dano'
end
Item.find_or_create_by(name: 'Escudo') do |item|
  item.kind = 'Espadachim'
  item.price = 150
  item.description = 'Absorve completamente o dano de armas pequenas e até 50% de armas grandes, com teste de agilidade. Pode absorver completamente dano de armas grandes com teste de força'
end
Item.find_or_create_by(name: 'Cajado') do |item|
  item.kind = 'Maga'
  item.price = 150
  item.description = 'Causa 1d4 de dano'
end
Item.find_or_create_by(name: 'Cajado Mágico') do |item|
  item.kind = 'Sacerdotisa'
  item.price = 150
  item.description = 'Causa 1d4 de dano'
end
